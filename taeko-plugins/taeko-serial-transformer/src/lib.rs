...
pub trait TaekoSerialTransformer: TaekoCoreDatabase {
    fn from_text<T>(&self, name: String) -> T
    where
        T: serde::de::DeserializeOwned;

    fn from_blob<T>(&self, name: String) -> T
    where
        T: serde::de::DeserializeOwned;
}
...
mod tests {
    ...
    use std::sync::Arc;
    use taeko_core::{salsa, TaekoCoreDatabaseStorage};
    ...
    mod json {
        ...
        impl TaekoSerialTransformer for JsonLoader {
            ...
            fn from_blob<T>(&self, _name: String) -> T
            where
                T: serde::de::DeserializeOwned,
            {
                todo!();
    ...
    fn json_works() {
        ...
        json.set_text(
            "./test_content/simple.json".to_string(),
            Arc::new(String::from(r#"{"name": "Taeko"}"#)),
        );
        let actual_map =
            json.from_text::<HashMap<String, String>>("./test_content/simple.json".to_string());
